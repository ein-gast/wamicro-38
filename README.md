...
По итогу проекта была написана [статья на Хабре](https://habr.com/ru/articles/1004054/). Там подробно разобраны некоторые технические нюансы.
...
Обсчёт вспышек света не влазит в 4К (примерно 500 лишних байт). В дефолтной сборке он отключен. 

Можно собрать приложение со светом, для этого нужно задать флаг `DFLAGS="-D WITH_LIGHT"` в `build.env` и выполнить `./build.sh`. Такая сборка будет подтормаживать, но будет ближе к исходной задумке.

## Готовая сборка

Ниже приведён собранный проект v1.0. Скопируйте data-url и вставьте в адресную строку браузера. К сожалению, у большинства мобильных браузеров нет поля для ввода URL, а только комплексное поле «поиск или URL», поэтому с телефона возможны трудности.
